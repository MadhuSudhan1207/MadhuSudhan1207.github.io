...
const validateUser= async()=>{
    ...
    <p onclick='showData(5)'><i class="bi bi-journal-album"></i> Todos</p> 
...
const getFeeds =async () =>{
    ...
    const json=await fetchData(url);
    let str="<div><h2> Feeds [All Poste]</h2>";
    json.map((element) => {
...
const getAlbums =async () =>{
    ...
    json.map((element) => {
        ...
        <p onclick=getimages(${element.id})>${element.title}</p> <hr>`;
...
const getProfile =async () =>{
    ...
    let str="<div><h2>Account Info </h2>";
    json.map((element) => {
...
/* const getimages= async (albumId)=>{
    ...
    const json=await fetchData(url);
    let str="<div><h2>Images </h2>";
    json.map((element) => {
        if(userId == element.albumId){
            str += `
